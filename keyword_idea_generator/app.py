...
from scraper.sitemap_parser import parse_sitemap
from scraper.google_autocomplete import get_autocomplete_keywords
from scraper.google_paa import get_paa_keywords
from scraper.google_trends import get_trends_keywords
import sqlite3
import os
import csv
...
def init_db():
    with sqlite3.connect(app.config['DATABASE_PATH']) as conn:
        ...
        conn.execute('''CREATE TABLE IF NOT EXISTS saved_keywords
...
def index():
    if request.method == 'POST':
        ...
        for keyword in seed_keywords:
            get_autocomplete_keywords(keyword.strip())
            get_paa_keywords(keyword.strip())
            get_trends_keywords(keyword.strip())
...
def save_keyword():
    try:
        ...
        with sqlite3.connect(app.config['DATABASE_PATH']) as conn:
            conn.execute('''INSERT OR IGNORE INTO saved_keywords 
...
def export_keywords():
    try:
        ...
        if export_type == 'all':
            ...
            keywords = []
            with open(app.config['KEYWORD_PATHS'][export_type], 'r', encoding='utf-8') as f:
                for line in f:
        ...
        return send_file(StringIO(output), as_attachment=True, attachment_filename='keywords.csv')
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500
